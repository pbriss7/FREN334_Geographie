...
En cliquant sur la flèche ![](images/clipboard-3589718497.png){width="20"} du bloc ci-dessous, Posit exécutera les lignes de commande et vous pourrez visualiser les six premières lignes de la structure de données.

```{r}
#| label: setup
#| echo: true

# L'option `include: true` signifie que le code et son résultat 
# seront affichés dans le document final.

# 1. Chargement des les bibliothèques nécessaires pour l'analyse
library(data.table) # Pour la manipulation rapide de grands jeux de données
library(sf)         # Pour la manipulation d'objets géospatiaux
library(ggplot2)    # Pour les graphiques statiques
library(leaflet)    # Pour les cartes interactives
library(viridis)    # Pour des palettes de couleurs accessibles
library(DT)         # Pour afficher des tableaux interactifs (si besoin)

# 2. Définition des noms de colonnes
noms_colonnes <- c(
...
# 3. Importation et nettoyage initial des données
...
Quand on travaille avec un tableau de données, il est utile d’avoir un vocabulaire simple et commun pour en parler. Trois mots-clés suffisent pour commencer : population, individus et variables.
...
Au croisement des lignes et des colonnes, donc des individus et des variables, se trouvent les **valeurs** ou **modalités**.

Une **valeur**, c’est l’information précise enregistrée pour un individu donné sur une variable donnée. • Exemple : si la variable est « âge », et que l’individu est Alice, la valeur peut être « 20 ».

Une **modalité** est un cas particulier : on utilise ce mot surtout pour les variables qualitatives. • Exemple : pour la variable « sexe », les modalités peuvent être « F » ou « M ». Pour la variable « ville de résidence », les modalités sont « Montréal », « Québec », « Trois-Rivières ».

Les **types de donnéées** Dans un tableau bien fait, toutes les valeurs d’une même variable doivent être du même type. C’est ce type qui détermine la nature des informations qu’on peut enregistrer et les opérations possibles.
...
Dans le bloc suivant, nous allons créer notre premier regroupement: nous allons d'abord observer les différentes modalités entrées par Melanie Conroy pour la variable `discourse`, puis nous allons calculer la fréquence de chacune de ces catégories.
...
Nous pourrions proposer le même examen pour chacune des variables catégorielles (c'est-à-dire les variables pour lesquelles les valeurs proposées correspondent à modalités). Essayez par exemple de remplacer `discourse` par `type_of_place` dans la dernière ligne de code, puis exécutez le bloc.

#### Notre premier diagramme à barres

Avec une table de fréquence comme celle que nous venons de composer, nous pouvons fabriquer un diagramme à barres. Chaque modalité de la variable correspondra à une barre et la hauteur de celle-ci sera déterminée par la fréquence (le nombre entier dans la deuxième colonne du tableau).
...
-   «Quels sont les lieux les plus ou les moins mentionnés dans les romans de Balzac?»
-   «Quel lieu est le plus ou le moins fréquemment mentionné?»

Ce type d'exploration, une donnée à la fois, à l'aide de diagrammes à barres ou de cartes géographiques, permet certes de repérer des tendances fortes ou faibles, mais il ne permet pas encore de comprendre le lien <i>entre</i> les variables.
...
Supposons par exemple que j'aie créé un jeu de données dans lequel se trouve dix romans de Balzac (1 par ligne) et que j'aie noté, pour chacun, le lieu du début du roman (lieu_debut) et le lieu où le roman se termine (lieu_fin).
...
Nous allons maintenant faire cela avec les données du tableau de Melanie Conroy.
